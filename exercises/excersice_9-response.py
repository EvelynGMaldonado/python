'''
EXCERCISE #9: RATE LIMITING

    Interview/Test Question might be something like -->  Finish line 18 by writing an expression that determines the value of the bucket.
    ...
    The code snippet below implements rate limiting using a "Token Bucket." Each time a message is received: the bucket is checked for tokens. 
    If the bucket has 1 or more tokens, the algorithm removes one token and sends the message. 
    If no tokens remain in the bucket, the algorithm drops the message.
...
def limit(self, callback_fn):
    ...
    # Use the following variables in your expression: time_passed, self.bucket, self.rate, and self.per 
    bucket = self.bucket + time_passed * (self.tokens / self.per)   # Insert your expression here
